...
export default function showModalDetails(card_id, all_result_events){
        ...
        for(let r=0; r<all_result_events.length; r++){
            if (all_result_events[r].id == card_id){
                // console.log(" AAAAAAAA");
                $('.modal-header').html(`<div class="card-title"><a href="#" target="_blank">${all_result_events[r].title}</a></div>`)
                $('.modal-event').html(`
                    
                    <div class="card-body">
                        <p><label>Category:</label> ${all_result_events[r].categories[0].title}</p>
                        <p><label>Date:</label> ${all_result_events[r].geometry[0].date}</p>
                        <p><label>Coordinates:</label> ${all_result_events[r].geometry[0].coordinates[0]} , ${all_result_events[r].geometry[0].coordinates[1]}</p>
                    </div>
                    <div class="card-footer">
                    <a href="${all_result_events[r].sources[0].url}" target="_blank"><button type="button" class="btn" style="background-color:#3aafeb; color:#f3f7f9">MORE INFO</button></a>
